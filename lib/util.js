...
export class BufferStream {
  constructor({input, encoding}) {
    if(typeof input === 'string') {
      ...
      this.bytes = Uint8Array.from(input);
    }
  ...
  write(val) {
    if(typeof val === 'string') {
      if(!this.encoding) {
        ...
        this.bytes = concatBuffers([this.bytes, buf]);
      } else {
        // FIXME: only support more as needed
        throw new TypeError(`"write()" with "${this.encoding}" not supported.`);
      }
    } else if(val instanceof Uint8Array) {
      this.bytes = concatBuffers([this.bytes, val]);
    } else {
      // FIXME: only support more as needed
      throw new TypeError('"val" must be a string or Uint8Array.');
    }
  }
 ...
 * error on invalid UTF8.
 */
const td = new TextDecoder('utf8', {fatal: true, ignoreBOM: false});

...
export class Tagged {
  ...
  _toTypedArray(val) {
    ...
    if((!little !== util.isBigEndian()) && (sz > 1)) {
  ...
  static _tag_2(v) {
    ...
    return util.bufferToBigNumber(v);
  }

  // Negative bignum; see Section 3.4.3
  static _tag_3(v) {
    // (note: replaced by bigint version in decoder.js when bigint on)
    const pos = util.bufferToBigNumber(v);
  ...
  static _tag_21(v) {
    if(util.isBufferish(v)) {
      this[INTERNAL_JSON] = () => base64url.encode(util.bufferishToBuffer(v));
    } else {
      setBuffersToJSON(v, function b64urlThis() { // no =>, honor `this`
        // eslint-disable-next-line no-invalid-this
        return base64url.encode(util.bufferishToBuffer(this));
      });
    }
    return this;
  }
  ...
  static _tag_22(v) {
    if(util.isBufferish(v)) {
      this[INTERNAL_JSON] = () => base64.encode(util.bufferishToBuffer(v));
    } else {
      setBuffersToJSON(v, function b64this() { // no =>, honor `this`
        // eslint-disable-next-line no-invalid-this
        return base64.encode(util.bufferishToBuffer(this));
      });
    }
    return this;
  }
  ...
  static _tag_23(v) {
    if(util.isBufferish(v)) {
      this[INTERNAL_JSON] = () => hex.encode(util.bufferishToBuffer(v));
    } else {
      setBuffersToJSON(v, function hexThis() { // no =>, honor `this`
        // eslint-disable-next-line no-invalid-this
        return hex.encode(util.bufferishToBuffer(this));
      });
    }
    return this;
  }

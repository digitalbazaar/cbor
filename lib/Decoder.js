...
import {MT, NUMBYTES, SYMS, BI, BN} from './constants.js';

const {BufferStream} = util;
...
export class Decoder {
  ...
  constructor(options = {}) {
    ...
    if(extendedResults) {
      throw new Error('"extendedResults" not implemented.');
  ...
  static decodeFirstSync(input, options = {}) {
    ...
    const s = util.guessEncoding(input, encoding);
  ...
  static decodeAll(input, options = {}, cb = null) {
    ...
    util.guessEncoding(input, encoding).pipe(c);
    return p;
  }

  // FIXME: no-op emit for now
  emit() {
  ...
  *_parse() {
    ...
    while(true) {
      ...
      const [octet] = yield 1;
      if(!this.running) {
        this.bs.unshift(new Uint8Array([octet]));
      ...
      switch(ai) {
        ...
        case NUMBYTES.EIGHT: {
          ...
          val = (mt === MT.SIMPLE_FLOAT) ?
            buf :
            util.parseCBORint(ai, buf, this.bigint);
          break;
        }
        case 28:
        case 29:
        ...
        case MT.UTF8_STRING:
          switch(val) {
            ...
            default:
              ...
              val = yield val;
              if(mt === MT.UTF8_STRING) {
                val = util.stringFromUtf8Bytes(val);
              } else {
      ...
      while(parent != null) {
        ...
        if(Array.isArray(parent)) {
        ...
        } else /* istanbul ignore else */ if(parent instanceof BufferStream) {
          // only parent types are Array and BufferStream for (Array/Map) and
          ...
          switch(parent[MAJOR]) {
            case MT.BYTE_STRING:
              ...
              break;
            case MT.UTF8_STRING:
              val = util.stringFromUtf8Bytes(parent.slice());
              break;
          }
        }
...
function parentBufferStream(parent, typ) {
  const b = new BufferStream({input: new Uint8Array()});
  b[COUNT] = -1;
...
function _tag_3(v) {
  ...
  return BI.MINUS_ONE - util.bufferToBigInt(v);
}

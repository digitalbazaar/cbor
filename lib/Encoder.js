...
const BUF_NAN = utils.hex('f97e00');
const BUF_INF_NEG = utils.hex('f9fc00');
const BUF_INF_POS = utils.hex('f97c00');
const BUF_NEG_ZERO = utils.hex('f98000');
...
export class Encoder {
  ...
  _pushString(obj) {
    const len = new TextEncoder().encode(obj).length;
  ...
  _pushBigint(obj) {
    ...
    if(this.collapseBigIntegers && obj.lte(constants.BN.MAXINT64)) {
  ...
  _pushArrayBuffer(gen, obj) {
    return gen._pushBuffer(gen, new Uint8Array(obj));
  }
  ...
  flush() {
    const result = utils.concat(this.stream);
    this.stream = [];
    return result;
  }
